<?php

include '../../config/config.php';
include '../classes/User.php';

$query = $_POST['query'];
$userLoggedIn = $_POST['userLoggedIn'];

$names = explode(" ", $query);

if (strpos($query, "_") !== false) {
	$usersReturned = mysqli_query($con, "SELECT * FROM users WHERE username LIKE '$query%' AND user_closed='no' LIMIT 8");
}else if(count($names) == 2){
	$usersReturned = mysqli_query($con, "SELECT * FROM users WHERE (first_name LIKE '%$names[0]%' AND last_name LIKE '%$names[1]%') AND user_closed='no' LIMIT 8");
}else{
	$usersReturned = mysqli_query($con, "SELECT * FROM users WHERE (first_name LIKE '%$names[0]%' OR last_name LIKE '%$names[0]%') AND user_closed='no' LIMIT 8");
}

if ($query != "") {

	while ($row = mysqli_fetch_array($usersReturned)) {

		$user = new User($con, $userLoggedIn);

		if ($row['username'] != $userLoggedIn) {
			$ortak_arkadaslar = $user->getOrtakFriends($row['username']) . " ortak arkadas";
		}else{
			$ortak_arkadaslar = "";
		}

		if ($user->isFriend($row['username'])) {
			echo "
				<div class='row p-0 m-0'>
					<div class='col-sm-12'>
						
							<a href='messages.php?u=". $row['username'] ."' style=''>
								<div class='media border p-3 border-0'>
									<img src='". $row['profile_pic'] ."' class='mr-3 mt-3 rounded-circle' style='width:80px;'>
									<div class='media-body'>
								      	<p style='font-size:20px;'>". $row['first_name'] ." ". $row['last_name'] ."</p>
								      	<p style='font-size:14px;'>". $row['username'] ."</p>
										<p style='font-size:10px;'>". $ortak_arkadaslar ."</p>
								    </div>
							    </div>
							</a>
						
					</div>
				</div>
			";
		}

	}

}

?>