...
class User{
	...
	// Her iki kullanicinin friend arrayinde diger kullanici silinerek guncelle
	...
	public function getOrtakFriends($user_to_check){
		$ortakFriends = 0;
		$user_array = $this->user['friend_array'];
		$user_array_explode = explode(",", $user_array); // delimiter, string

		$query = mysqli_query($this->con, "SELECT friend_array FROM users WHERE username='$user_to_check'");
		$row = mysqli_fetch_array($query);
		$user_to_check_array = $row['friend_array'];
		$user_to_check_array_explode = explode(",", $user_to_check_array); // delimiter, string
