...
class Notification{
	...
	public function getNotificationsDropdown($data, $limit){
		...
		$query = mysqli_query($this->con, "SELECT * FROM notifications WHERE user_to='$userLoggedIn' ORDER BY notification_id DESC");
		...
		while ($row = mysqli_fetch_array($query)) {
			...
			$return_string .= "
				<a href='". $row['link'] ."'>
					<div class='displayNotification' style='". $style ."'>
						<div>
							<img src='". $user_data['profile_pic'] ."'>
						</div>
						<p class='timestamp-notification'>". $time_message ."</p>
						<p>". $row['message'] ."</p>
					</div>
				</a>
			";
	...
	public function insertNotification($post_id, $user_to, $type){
		...
		switch ($type) {
			case 'comment':
				$msg = $userLoggedInName . " commented on your post";
				break;
			case 'like':
				$msg = $userLoggedInName . " liked your post";
				break;
			case 'profile_post':
				$msg = $userLoggedInName . " posted on your profile";
				break;
			case 'comment_non_owner':
				$msg = $userLoggedInName . " commented on a post you commented on";
				break;
			case 'profile_comment':
				$msg = $userLoggedInName . " commented on your profile post";
				break;
		}

		$link = "post.php?id=" . $post_id;
		$insert_query = mysqli_query($this->con, "INSERT INTO notifications VALUES('', '$user_to', '$userLoggedIn', '$msg', '$link', '$date_time', 'no', 'no')");
	}
